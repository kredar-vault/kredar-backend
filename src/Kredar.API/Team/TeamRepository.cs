...
public class TeamRepository(AppDbContext db)
{
    ...
    public async Task<TeamMember?> FindByEmailGlobalAsync(string email) =>
        await db.TeamMembers.FirstOrDefaultAsync(t => t.Email == email && t.Status == TeamMemberStatus.Active);
